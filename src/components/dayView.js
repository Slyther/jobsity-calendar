        ...
        .map(reminder => {
            return (
              <Reminder
                ...
                reminder={reminder}
                onEdit={() => { this.props.getReminder(reminder); this.props.openModal() }}
                onDelete={() => {
                  this.setState(
                      {
    ...
    return (
      <div className="day-view-container">
        <Nav fill className="justify-content-center" activeKey="/home">
            <Nav.Item>
            <Button variant="light" onClick={() => this.props.goToDay(moment(this.props.currentView.payload, 'MM/DD/YYYY').subtract(1, 'days').format('MM/DD/YYYY'))}>&lt;</Button>
            </Nav.Item>
            <Nav.Item>
                <h5 className="text-center">{moment(this.props.currentView.payload, 'MM/DD/YYYY').format('dddd, MMMM Do, YYYY')}</h5>
            </Nav.Item>
            <Nav.Item>
            <Button variant="light" onClick={() => this.props.goToDay(moment(this.props.currentView.payload, 'MM/DD/YYYY').add(1, 'days').format('MM/DD/YYYY'))}>&gt;</Button>
            </Nav.Item>
        </Nav>
        ...
        <ConfirmationModal
          ...
          onCancel={() => this.setState({showConfirmationModal: false, deleteAll: true, reminderToDelete: -1, selectedDate: ''})}
          onDeleteAll={() => this.props.deleteReminders(this.state.selectedDate)}
          onDeleteOne={() => this.props.deleteReminder(this.state.reminderToDelete)}
        />
        <Nav fill className="justify-content-center" activeKey="/home">
            <Nav.Item>
